...
| **Climate program labels** | Per `device_id`: typed rows override **CT_*_\<tstat\>*** labels by **climateList** index. **Cloud** thermostats use the full list for **GV3** status and commands. **HomeKit** thermostats use the full list only for **GV3** **status** (**CTA_***) so uncommon values still resolve; **GV3** **commands** use a four-option **CT_HK_*** editor (**away / home / sleep / smart1**, IoX indices **0–3** with the default catalog). |
...
**Still present (supported via hub):** **ST**, **CLISPH**, **CLISPC**, **CLIMD**, **CLIFS**, **CLIHUM**, **CLIHCS**, **CLIFRS**, **GV1** (humidification setpoint / target relative humidity), **GV3** (comfort / program: **read** from ``VENDOR_ECOBEE_CURRENT_MODE``; **set** via ``VENDOR_ECOBEE_SET_HOLD_SCHEDULE``). **CLIFS** uses the **same IoX values as cloud** (``fanMap``: **auto = 0**, **on = 1**); HAP **TargetFanState** (**0 = On, 1 = Auto**) is translated at the hub boundary like cloud.

**GV3 (HomeKit) — four hold slots only:** Ecobee’s vendor hold characteristic accepts **four** program bytes (**0 = Home, 1 = Sleep, 2 = Away, 3 = Temp** on the wire), which map to IoX **climateList** indices **away (0), home (1), sleep (2), smart1 (3)**. The profile therefore exposes only those **four** choices on **GV3** **commands** (**CT_HK_***). **GV3** **status** still uses the full **CTA_*** range so labels match if the driver ever holds another index (e.g. after a backend switch). Vacation, Smart Away, and other app-only programs are **not** separate HomeKit writes—use the **Ecobee app** or **cloud** backend for those. **BRT** / **DIM** (setpoint step via target temperature where applicable), **QUERY**.
...
**Comfort / schedule:** The Ecobee app’s full program list (vacation, custom comforts, Smart Home/Away, etc.) is not exposed as distinct HomeKit hold values—only **Home, Sleep, Away,** and **Temp** (mapped to **smart1** in IoX). Additional cloud-only behavior may never map 1:1 to HomeKit (schedules, hold duration, some comfort features). Expect to iterate on firmware and hub protocol as you test.
