...
from node_funcs import climateList
...
def test_homekit_climate_details_override():
    rows = [{'device_id': 'aa:bb', 'index': 1, 'name': 'Custom'}]
...
def test_homekit_climate_details_skips_indices_when_requested():
    rows = [{'device_id': 'aa:bb', 'index': 1, 'name': 'HubSlot1'}]
    d = homekit_climate_details_for_device(
        'AA:BB', rows, climateList[:5], skip_catalog_indices={0, 1, 2, 3}
    )
    assert d[1]['name'] != 'HubSlot1'
    assert d[1]['name'] == (climateList[1][0].upper() + climateList[1][1:] if climateList[1] else '1')
