...
from udi_interface import LOGGER, LOG_HANDLER
...
class CloudBackend:
    """Cloud (REST/OAuth) backend logic; runs against the dispatcher :class:`Controller` node."""
    ...
    def handler_start(self):
        ...
        self.use_oauth = bool(isinstance(init, dict) and init.get('oauth'))
        self.heartbeat()
        #
    ...
    def handler_nsdata(self, key, data):
        ...
        try:
            ...
            if not isinstance(data, dict):
                LOGGER.error('nsdata must be a dict or JSON object, got %s', type(data).__name__)
                self.handler_nsdata_st = False
                return
            if self.use_oauth:
    ...
    def handler_data(self,data):
        ...
        if 'tokenData' in payload:
            self.tokenData = payload['tokenData']
    ...
    def check_profile(self,thermostats):
        ...
        if update_profile:
            write_ecobee_climate_profile(
                Path('.'),
                climates,
                log_prefix='{}:write_profile:'.format(self.address),
                logger=LOGGER,
            )
